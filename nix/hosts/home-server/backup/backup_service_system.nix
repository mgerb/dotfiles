# Root mode service to run remote backup on a timer. This will show incoming/outgoing IP data
# in the webhook.
{
  pkgs,
  user,
  ...
  jobName = backupNames.systemJobName;
in {
  ...
  systemd.services."${jobName}Stop" = {
    ...
    serviceConfig = {
      ...
      ExecStartPost = "${pkgs.bash}/bin/bash /raid-pool/samba/backup_finished_webhook.sh '-u ${jobName}.service'";
    };
  };
